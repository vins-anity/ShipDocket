        ...
        <div className="space-y-6 animate-fade-in max-w-5xl mx-auto py-4">
            ...
            <Tabs defaultValue="general" className="space-y-6">
                <TabsList className="bg-brand-light/5 border border-brand-gray-light/20 p-1 flex-wrap h-auto rounded-xl">
                    <TabsTrigger value="general" className="data-[state=active]:bg-brand-dark data-[state=active]:text-brand-light text-brand-gray-mid hover:text-brand-dark transition-all">
                        <IconUser className="w-4 h-4 mr-2" /> General
                    </TabsTrigger>
                    <TabsTrigger value="workflow" className="data-[state=active]:bg-brand-dark data-[state=active]:text-brand-light text-brand-gray-mid hover:text-brand-dark transition-all">
                        <IconSettings className="w-4 h-4 mr-2" /> Workflow
                    </TabsTrigger>
                    <TabsTrigger value="proofs" className="data-[state=active]:bg-brand-dark data-[state=active]:text-brand-light text-brand-gray-mid hover:text-brand-dark transition-all">
                        <IconCheck className="w-4 h-4 mr-2" /> Proof Rules
                    </TabsTrigger>
                    <TabsTrigger value="health" className="data-[state=active]:bg-brand-dark data-[state=active]:text-brand-light text-brand-gray-mid hover:text-brand-dark transition-all">
                        <IconActivity className="w-4 h-4 mr-2" /> System Health
                    </TabsTrigger>
                    <TabsTrigger value="team" className="data-[state=active]:bg-brand-dark data-[state=active]:text-brand-light text-brand-gray-mid hover:text-brand-dark transition-all">
                        <IconUsers className="w-4 h-4 mr-2" /> Team
                    </TabsTrigger>
                    <TabsTrigger value="branding" className="data-[state=active]:bg-brand-dark data-[state=active]:text-brand-light text-brand-gray-mid hover:text-brand-dark transition-all">
                ...
                <TabsContent value="workflow" className="space-y-4">
                    <Card className="bg-white border-brand-gray-light shadow-sm rounded-xl overflow-hidden">
                        <CardHeader className="bg-brand-light/30 border-b border-brand-gray-light/30 pb-4">
                            <CardTitle className="text-xl font-heading font-bold text-brand-dark">Trigger Configuration</CardTitle>
                            <CardDescription className="text-brand-gray-mid font-serif italic">When should ShipDocket start tracking?</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6 pt-6">
                            <div className="flex flex-wrap gap-2">
                                {startStatuses.map((status) => (
                                    <div key={status} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-brand-accent-blue/10 border border-brand-accent-blue/20 text-brand-accent-blue text-sm font-bold shadow-sm">
                                        <span>{status}</span>
                            ...
                            <div className="flex gap-2 max-w-sm">
                                ...
                                />
                                <Button onClick={(e) => { e.preventDefault(); if (newStatus && !startStatuses.includes(newStatus)) { setStartStatuses([...startStatuses, newStatus]); setNewStatus(""); } }} variant="secondary" className="bg-brand-dark text-brand-light hover:bg-black">Add</Button>
                            </div>
                            <Button onClick={handleSaveStartStatuses} className="mt-4 bg-brand-accent-blue text-white hover:bg-blue-600 font-bold shadow-md shadow-blue-500/20 rounded-lg">
                ...
                <TabsContent value="proofs" className="space-y-4">
                    <Card className="bg-white border-brand-gray-light shadow-sm rounded-xl overflow-hidden">
                        <CardHeader className="bg-brand-light/30 border-b border-brand-gray-light/30 pb-4">
                            <CardTitle className="text-xl font-heading font-bold text-brand-dark">Smart Creation Rules</CardTitle>
                            <CardDescription className="text-brand-gray-mid font-serif italic">Configure when Proof Packets are automatically generated.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6 pt-6">
                            <div className="flex items-center justify-between p-5 rounded-xl bg-brand-light/40 border border-brand-gray-light/30">
                                <div>
                                    <h3 className="font-bold text-brand-dark text-lg">Auto-Create on "Done"</h3>
                                ...
                                >
                                    <div className={`w-6 h-6 rounded-full bg-white shadow-sm transition-transform ${autoCreateOnDone ? 'translate-x-6' : 'translate-x-0'}`} />
                                </div>
                            </div>
                            <Button onClick={handleSaveRules} className="bg-brand-accent-green hover:bg-green-600 text-white font-bold shadow-md shadow-green-500/20 rounded-lg">
                ...
                <TabsContent value="team" className="space-y-4">
                    <Card className="bg-white border-brand-gray-light shadow-sm rounded-xl overflow-hidden">
                        <CardHeader className="flex flex-row items-center justify-between bg-brand-light/30 border-b border-brand-gray-light/30 pb-4">
                            <div>
                                <CardTitle className="text-xl font-heading font-bold text-brand-dark">Team Members</CardTitle>
                                <CardDescription className="text-brand-gray-mid font-serif italic">Manage access to your workspace.</CardDescription>
                            </div>
                            <Button variant="outline" className="border-brand-gray-light text-brand-dark hover:bg-brand-light">Invite Member</Button>
                        </CardHeader>
                        <CardContent className="pt-6">
                            <div className="space-y-3">
                                {members?.map((m: any) => (
                                    <div key={m.userId} className="flex items-center justify-between p-4 rounded-xl bg-white border border-brand-gray-light/40 hover:border-brand-gray-light hover:shadow-sm transition-all group">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="text-sm font-bold text-brand-dark group-hover:text-black transition-colors">{m.userId}</div>
                                                <div className="text-xs text-brand-gray-mid font-medium uppercase tracking-wide">{m.role}</div>
                                            </div>
                                        </div>
                                        <Badge variant={m.role === 'owner' ? 'default' : 'secondary'} className={m.role === 'owner' ? "bg-brand-dark text-brand-light" : "bg-brand-gray-light/30 text-brand-dark"}>
                                            {m.role}
                                        </Badge>
                                    </div>
                                ))}
                                {!members?.length && <div className="text-brand-gray-mid/60 text-sm font-serif italic p-4 text-center">No members found.</div>}
                            </div>
                        </CardContent>
                    </Card>
                </TabsContent>
                ...
                <TabsContent value="branding" className="space-y-4">
                    <Card className="bg-white border-brand-gray-light shadow-sm rounded-xl overflow-hidden">
                        <CardHeader className="bg-brand-light/30 border-b border-brand-gray-light/30 pb-4">
                            <CardTitle className="text-xl font-heading font-bold text-brand-dark">Whitelabel Settings</CardTitle>
                            <CardDescription className="text-brand-gray-mid font-serif italic">Customize the Proof Packets sent to your clients.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6 pt-6">
                            <div className="grid gap-6 max-w-md">
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-brand-dark">Primary Brand Color</label>
                                    <div className="flex gap-3">
                                        <div className="w-10 h-10 rounded-lg border border-brand-gray-light shadow-sm" style={{ background: brandColor }} />
                                        <Input value={brandColor} onChange={e => setBrandColor(e.target.value)} className="bg-brand-light border-brand-gray-light font-mono text-sm" />
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-brand-dark">Logo URL</label>
                                    <Input value={brandLogo} onChange={e => setBrandLogo(e.target.value)} placeholder="https://..." className="bg-brand-light border-brand-gray-light" />
                                </div>
                            </div>
                            <Button onClick={handleSaveBranding} className="bg-brand-accent-orange hover:bg-orange-600 text-white font-bold shadow-md shadow-orange-500/20 rounded-lg">
            ...
            <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                    ...
                    {connected ? (
                        ...
                        <Badge variant="destructive" className="bg-red-50/50 text-red-500 border-red-100 px-2 py-0.5 uppercase text-[10px] font-bold tracking-wider">
