...
describe("JiraAnalyzerService", () => {
    ...
    it("should detect Kanban workflow when kanban board is present", async () => {
        // Setup Mocks
        (jiraService.callJiraAPI as any).mockImplementation(
            (_cloudId: string, _token: string, endpoint: string) => {
                if (endpoint.includes("/project"))
                    return Promise.resolve([{ projectTypeKey: "software" }]);
                if (endpoint.includes("/board"))
                    return Promise.resolve({ values: [{ type: "kanban", name: "Kanban Board" }] });
                ...
                if (endpoint.includes("/issuetype"))
                    return Promise.resolve([{ name: "Task", subtask: false }]);
                return Promise.resolve([]);
            },
        );
        ...
        (jiraService.callJiraAPI as any).mockImplementation(
            (_cloudId: string, _token: string, endpoint: string) => {
                if (endpoint.includes("/project"))
                    return Promise.resolve([{ projectTypeKey: "software" }]);
                if (endpoint.includes("/board"))
                    return Promise.resolve({ values: [{ type: "scrum", name: "Scrum Board" }] });
                if (endpoint.includes("/status"))
                    return Promise.resolve([
                ...
                if (endpoint.includes("/issuetype"))
                    return Promise.resolve([{ name: "Story", subtask: false }]); // Stories boost confidence
                return Promise.resolve([]);
            },
        );
    ...
    it("should fallback to Project Management if no boards but business project detected", async () => {
        ...
        (jiraService.callJiraAPI as any).mockImplementation(
            (_cloudId: string, _token: string, endpoint: string) => {
                if (endpoint.includes("/project"))
                    return Promise.resolve([{ projectTypeKey: "business" }]);
        ...
        (jiraService.callJiraAPI as any).mockImplementation(
            (_cloudId: string, _token: string, endpoint: string) => {
                if (endpoint.includes("/project"))
                    return Promise.resolve([{ projectTypeKey: "software" }]);
                if (endpoint.includes("/board"))
                    return Promise.resolve({ values: [{ type: "kanban" }] });
                if (endpoint.includes("/status"))
                    return Promise.resolve([
                        { name: "In Progress", statusCategory: { key: "indeterminate" } },
